...
export function updateTimeAndCountdown() { 
    ...
    try {
        ...
        if(clockDateEl) clockDateEl.textContent = now.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });
    } catch(e) {}
    ...
    if (savedStart) {
        ...
        if (now < tripStart) {
            ...
            if ([100, 50, 30, 10, 5, 1].includes(days) && !localStorage.getItem(`milestone_${days}`)) {
                localStorage.setItem(`milestone_${days}`, 'true');
