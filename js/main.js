import { state, setVal, getVal, parseDateTime } from './store.js?v=6.2.0';
import { loadAllData, initLiveCurrency, preCacheImages, syncToCloud, deleteQuoteFromSheet } from './api.js?v=6.2.0';
...
function startSyncHeartbeat() {
    // RUNS EVERY 60 SECONDS TO CHECK FOR NEW ANNOUNCEMENTS
    setInterval(async () => {
        await loadAllData();
        renderMeetupBoard();
...
function bindEvents() {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#btn-close-urgent')) {
            ...
            // Mark the meetup as seen so the board stops pulsing
        ...
        if (e.target.closest('#btn-open-meetup')) { 
            openMeetupModal(); 
            // Mark current as seen when opening modal
            const meetups = (state.quotesData || []).filter(q => q[0] === 'MEETUP');
            if (meetups.length > 0) {
                const latest = meetups[meetups.length - 1];
                const messageId = btoa(latest[1] + latest[2]).substring(0, 12);
        ...
        if (e.target.closest('#btn-find-car')) {
            ...
            window.open(`https://www.google.com/maps/search/?api=1&query=\${btn.dataset.lat},${btn.dataset.lon}&travelmode=walking`, '_blank');
            return;
        }
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=6.2.0');
