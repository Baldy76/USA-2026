...
export function openTab(pageId) {
    ...
    if(targetPage) targetPage.classList.add('active', 'fade-pop');
    ...
    document.addEventListener('touchend', e => {
        if (window.scrollY === 0 && pStart > 0) {
            if (e.changedTouches[0].clientY - pStart > 150) {
                if(spinner) spinner.classList.add('refreshing'); 
...
function bindEvents() {
    ...
    document.body.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#btn-clear-anchor')) { 
            e.stopPropagation(); 
            if(confirm("Pull up anchor?")) { localStorage.removeItem('carAnchor'); renderAnchor(); } 
            return; 
        }
        ...
        if (e.target.closest('#btn-enable-notifs')) {
            ...
            if ('Notification' in window && window.Notification) {
                Notification.requestPermission().then(permission => {
                    ...
                    } else { alert('Notifications denied by device.'); }
                });
            } else { alert('Push notifications are not supported on this browser/device.'); }
            return;
        }
        ...
        if (e.target.closest('#clear-usd')) { 
            ...
            if (usdInput) usdInput.value = ''; convertCurrency(); return; 
        }
        ...
        if (e.target.closest('#btn-force-sync')) {
            ...
            setTimeout(() => { btn.innerText = "☁️ Sync Data"; }, 2000); 
            return;
        }
        ...
        if (travelCard && !e.target.closest('.edit-gate-btn')) { 
        ...
        if (activeCard && e.target.tagName !== 'A') { 
        ...
        if (completedCard && e.target.tagName !== 'A') {
            let tasks = await getVal('completedTasks') || []; 
            tasks = tasks.filter(id => id !== completedCard.dataset.taskId);
            await setVal('completedTasks', tasks); 
            renderItinerary(); 
            renderUpNext(); 
            return;
        }
