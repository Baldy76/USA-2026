...
export async function renderItinerary() {
    ...
    sortedData.forEach(cols => {
        ...
        if (isMatch) {
            ...
            // THE FIX: Exact official Google Maps Native Search API string format
            ...
            if (loc.toLowerCase().includes('la')) { 
                if (isCompleted) cLA += cardHtml; else { if (!grouped['la'][d]) grouped['la'][d] = []; grouped['la'][d].push(cardHtml); }
            } else if (loc.toLowerCase().includes('utah')) { 
                if (isCompleted) cUtah += cardHtml; else { if (!grouped['utah'][d]) grouped['utah'][d] = []; grouped['utah'][d].push(cardHtml); }
            } else if (loc.toLowerCase().includes('vegas')) { 
                if (isCompleted) cVegas += cardHtml; else { if (!grouped['vegas'][d]) grouped['vegas'][d] = []; grouped['vegas'][d].push(cardHtml); }
            }
        }
    });
    ...
    document.getElementById('la-completed-list').innerHTML = cLA;
    document.getElementById('utah-itinerary').innerHTML = buildSec(grouped['utah']); 
    document.getElementById('utah-completed-list').innerHTML = cUtah;
    document.getElementById('vegas-itinerary').innerHTML = buildSec(grouped['vegas']); 
    document.getElementById('vegas-completed-list').innerHTML = cVegas;
}
            ...
            <div class="flip-container travel-card">
                <div class="flip-card-inner">
                    <div class="flip-front" style="background: linear-gradient(135deg, #0284c7, #0369a1); color: white; border: none; padding:20px; border-radius:24px;">
                        <div style="font-size: 11px; font-weight: 900; opacity: 0.7; text-transform: uppercase;">✈️ ${escapeHTML(cols[1].toUpperCase())} • ${escapeHTML(cols[2])}</div>
...
export function renderAccommodations() { 
    ...
    state.vaultAndStaysData.forEach(cols => {
        ...
        if (type === 'stay') {
            ...
            const html = `
            <div class="admin-card stay-card" data-fam="${fam}" data-addr="${addr}" data-map="${mapLink}" data-link="${link}" data-img="${img}" style="padding: 0; overflow: hidden; margin-bottom: 24px; cursor: pointer;">
                <div style="height: 100px; background: ${img?`url('${img}') center/cover`:`var(--accent)`}; display: flex; align-items: flex-end; padding: 20px;">
                    <h3 style="margin: 0; color: white; font-size: 20px; text-shadow: 0 2px 10px rgba(0,0,0,0.5); font-weight: 900;">🏡 ${fam} Stay</h3>
                </div>
            </div>`;
            
            const city = (cols[3] || '').toLowerCase();
            if(city.includes('la')) grouped['la'] += html; 
            else if(city.includes('utah')) grouped['utah'] += html; 
            else if(city.includes('vegas')) grouped['vegas'] += html;
        }
    });
