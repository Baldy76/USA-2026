...
export function triggerJackpotMode() {
    ...
    setTimeout(() => { overlay.style.opacity = '0'; overlay.style.transition = 'opacity 0.5s ease'; setTimeout(() => overlay.remove(), 500); }, 3500);
}

// ------------------------------------------------------------------
// 💡 THE NEW TIPS ENGINE (Reads from state.hintsData)
// ------------------------------------------------------------------
...
export function renderVegasFoodList(typeFilter) {
    ...
    data.forEach(row => {
        ...
        html += `<div style="background: var(--card); padding: 15px; border-radius: 12px; margin-bottom: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border: 1px solid rgba(0,0,0,0.05);">
...
function matchUser(whoStr, userFilter) {
    const w = whoStr.toLowerCase(); const u = userFilter.toLowerCase();
    if (u === 'all' || w === 'everyone' || w === '') return true;
    if (w.includes(u) || u.includes(w)) return true;
    const leech = ['graeme', 'dawn', 'grace']; const murray = ['david', 'sarah', 'bexs'];
...
export function openRoadtripModal() {
    ...
    // Bulletproof extraction: Ignore blanks, and totally ignore "National Park" headers
    const routes = [...new Set(data.map(row => (row[0] || '').trim()).filter(r => 
        r !== '' && 
        !r.toLowerCase().includes('national park') && 
...
export function renderRoadtripList(selectedRoute) {
    ...
    state.roadtripData.forEach(cols => {
        ...
        // Bulletproof skip headers - literally ignores anything containing "Location / Activity"
        if (!activity || activity.toLowerCase().includes('location / activity') || time.toLowerCase().includes('local time')) return; 
        
        let icon = '📍';
        const actLower = activity.toLowerCase() + ' ' + details.toLowerCase();
        if (actLower.includes('fuel') || actLower.includes('gas') || actLower.includes('maverik') || actLower.includes('chevron')) icon = '⛽';
