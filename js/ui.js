...
export function clearCustomFamilies() { if(confirm("Remove all old saved names?")) { localStorage.removeItem('customFamilies'); localStorage.removeItem('appUser'); window.location.reload(); } }
...
export function renderQuotes(location) {
    ...
    list.innerHTML = filtered.map(q => `<div class="admin-card" style="padding: 15px; margin-bottom: 12px; background: rgba(0,0,0,0.03); border: 1px solid var(--ios-grey);"><div style="font-family: 'Georgia', serif; font-style: italic; font-size: 16px; line-height: 1.4; margin-bottom: 8px;">"${escapeHTML(q[1])}"</div><div style="text-align: right; font-size: 11px; font-weight: 800; opacity: 0.6; text-transform: uppercase;">— ${escapeHTML(q[2])}</div></div>`).reverse().join('');
}
