...
export function spinRoulette() {
    ...
    setTimeout(() => { btn.disabled = false; triggerConfetti(); if(navigator.vibrate) navigator.vibrate([30, 50, 30]); }, 4500);
}
...
export function openStayModal(fam, addr, imgUrl, listLink) {
    ...
    document.getElementById('stay-modal-buttons').innerHTML = `<button class="action-btn link-btn" data-url="${"https://www.google.com/maps/dir/?api=1&destination="+encodeURIComponent(addr)}" style="flex:1;">Drive</button>${listLink?`<button class="action-btn link-btn" data-url="${listLink}" style="flex:1;">Listing</button>`:''}`;
    document.getElementById('stay-modal').style.display = 'flex'; setTimeout(() => document.getElementById('stay-modal').classList.add('active'), 10);
}
...
export function openTravelModal(cardData) {
    ...
    document.getElementById('travel-modal').style.display = 'flex'; setTimeout(() => document.getElementById('travel-modal').classList.add('active'), 10);
}
