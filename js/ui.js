...
export function updateTimeAndCountdown() { 
    try {
        ...
        if(elLA) elLA.innerText = `🕒 Local Time: ${timePT}`; if(elVegas) elVegas.innerText = `🕒 Local Time: ${timePT}`; if(elUtah) elUtah.innerText = `🕒 Local Time: ${timeMT}`;
        ...
        if (savedStart) {
            ...
            if (!isNaN(tripDate.getTime())) {
                ...
                if (diff > 0) {
                    ...
                    const cdText = document.getElementById('cd-text'); if(cdText) cdText.innerHTML = `🚀 ${days} Days to Go!`;
...
export function clearCustomFamilies() {
    if(confirm("Remove all old saved names from this device?")) {
        localStorage.removeItem('customFamilies'); populateDropdown(); updateFamilyFilter();
    }
}
...
export async function initWeatherPill() {
    const loadSummary = async (lat, lon, id) => {
        try {
            const data = await fetchWeather(lat, lon); const el = document.getElementById(id);
            if(el) el.innerHTML = `${getWeatherIcon(data.current.weather[0].icon)} ${Math.round(data.current.main.temp)}°`;
        } catch(e) { const el = document.getElementById(id); if(el) el.innerHTML = '🚫'; }
    };
    loadSummary(34.0522, -118.2437, 'wp-la'); loadSummary(37.0965, -113.5684, 'wp-utah'); loadSummary(36.1699, -115.1398, 'wp-vegas');
    if (navigator.geolocation) { navigator.geolocation.getCurrentPosition(pos => loadSummary(pos.coords.latitude, pos.coords.longitude, 'wp-local'), err => { const el = document.getElementById('wp-local'); if(el) el.innerHTML = '🚫'; }); }
}
    ...
    try {
        ...
        if (target === 'utah') { lat = 37.0965; lon = -113.5684; locName = "Utah"; }
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(async (pos) => { renderWeatherDOM(await fetchWeather(pos.coords.latitude, pos.coords.longitude), "Local GPS"); }, async (err) => { renderWeatherDOM(await fetchWeather(lat, lon), locName); }, { timeout: 5000 }); return;
            }
        }
...
export function openWeatherModal() {
    const modal = document.getElementById('weather-modal'); modal.style.display = 'flex'; setTimeout(() => modal.classList.add('active'), 10);
...
export function closeWeatherModal() { const modal = document.getElementById('weather-modal'); modal.classList.remove('active'); setTimeout(() => modal.style.display = 'none', 300); }
...
export async function renderItinerary() {
    ...
    state.itineraryData.forEach(cols => {
        ...
        if (isMatch) {
            ...
            const cardHtml = `<div class="admin-card itin-card ${isCompleted?'completed':''}" data-task-id="${taskId}" data-task-name="${escapeHTML(act)}" style="text-align: left; transition: all 0.3s ease; cursor: pointer; padding: 20px; margin-bottom: 16px;"><div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid var(--ios-grey); padding-bottom: 10px; margin-bottom: 10px;"><strong style="font-size: 15px; font-weight: 800;">${escapeHTML(time)}</strong>${badgeHtml}</div><div class="itin-title" style="font-size: 17px; font-weight: 900; line-height: 1.3; margin-bottom: 8px;">${escapeHTML(act)}</div><div style="font-size: 14px; font-weight: 700; opacity: 0.7; line-height: 1.5;">📍 <a href="${mapLink}" target="_blank" style="color: var(--accent); text-decoration: none;">Get Directions</a>${addrHtml}</div></div>`;
...
export function openTravelModal(cardData) {
    ...
    if (cardData.type === 'flight') {
        ...
        <div class="flight-card" style="margin-bottom:0; box-shadow:none;">
            <div class="flight-header"><span class="flight-num">${linkHtml}</span><span style="font-size:12px; font-weight:800; opacity:0.8; text-align: right;">${cardData.date} <br> TIME: ${cardData.ftime}</span></div>
            <div class="flight-path" style="margin: 25px 0;"><div class="path-node"><span>From</span><strong style="font-size: 20px;">${cardData.dep}</strong></div><div class="plane-icon"></div><div class="path-node"><span>To</span><strong style="font-size: 20px;">${cardData.arr}</strong></div></div>
            <div style="margin-top:15px; display:flex; justify-content: space-between; align-items:center; font-size:13px; font-weight:700; opacity:0.9;"><span>Term/Gate: <strong id="modal-gate-text" style="color: #ffd60a;">${cardData.term || "Check Screens"}</strong> <button class="edit-gate-btn action-btn" data-flightid="${cardData.flightid}" style="padding: 4px 10px; font-size: 11px; width: auto; margin: 0 0 0 10px; display: inline-block; background: rgba(255,255,255,0.2); color: white; box-shadow: none; border: 1px solid rgba(255,255,255,0.4);">✏️ Update</button></span><span>Ref: ${cardData.ref}</span></div>
            <div class="barcode"></div>
        </div>`;
    } else if (cardData.type === 'car') {
        wrapper.style.background = 'var(--card)'; wrapper.style.border = '1px solid var(--ios-grey)'; wrapper.style.boxShadow = '0 8px 24px var(--shadow)';
        content.innerHTML = `
        <div style="padding: 24px;"><div style="font-size:12px; font-weight:900; opacity:0.5; text-transform:uppercase; letter-spacing: 1px; margin-bottom: 15px;">🚗 Car Rental</div><h3 style="margin: 0 0 20px; font-size: 24px; font-weight: 900;">${cardData.company}</h3><div style="margin-bottom: 16px; padding-left: 12px; border-left: 3px solid var(--ios-grey);"><strong style="font-size: 14px;">Pick-up:</strong><br><span style="font-size: 16px; font-weight: 800;">${cardData.ploc}</span><br><span style="font-size:14px; font-weight:600; opacity:0.7;">${cardData.pdate} @ ${cardData.ptime}</span></div><div style="margin-bottom: 20px; padding-left: 12px; border-left: 3px solid var(--ios-grey);"><strong style="font-size: 14px;">Drop-off:</strong><br><span style="font-size: 16px; font-weight: 800;">${cardData.dloc}</span><br><span style="font-size:14px; font-weight:600; opacity:0.7;">${cardData.ddate} @ ${cardData.dtime}</span></div><div style="background: rgba(0,0,0,0.05); padding: 12px; border-radius: 12px; font-size: 15px;"><strong>Ref:</strong> <span style="color: var(--accent); font-weight: 800;">${cardData.ref}</span></div></div>`;
    }
    modal.style.display = 'flex'; setTimeout(() => modal.classList.add('active'), 10);
}
