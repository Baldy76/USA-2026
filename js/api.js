...
export async function loadAllData() {
    ...
    try {
        ...
        const [itin, vault, quotes] = await Promise.all([fetchCSV(urls.itinerary), fetchCSV(urls.vault), fetchCSV(urls.quotes)]);
        ...
        await Promise.all([setVal('itin_cache', state.itineraryData), setVal('vault_cache', state.vaultAndStaysData), setVal('quotes_cache', state.quotesData)]);
    } catch (e) {
...
export function syncToCloud() { console.log("Sync Active"); }
export function initLiveCurrency() { console.log("Currency Active"); }
export function preCacheImages() { console.log("Images Active"); }
