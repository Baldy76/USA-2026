...
function applyTheme(isDark) {
    ...
    if (meta) {
        meta.content = isDark ? "#0b0e14" : "#f2f2f7";
    }
...
// AUDITED: Replaced error-prone quote replacements with absolutely safe HTML entity codes.
        ...
        if (fParts.length >= 3) {
            ...
            <div class="flight-card">
                <div class="flight-header">
                    <span class="flight-num">FLIGHT ${escapeHTML(fParts[2])}</span>
                    <span style="font-size:12px; font-weight:800; opacity:0.8;">BOARDING PASS</span>
                </div>
                <div class="flight-path">
                    <div class="path-node"><span>From</span><strong>${escapeHTML(fParts[0])}</strong></div>
                    <div class="plane-icon"></div>
                    <div class="path-node"><span>To</span><strong>${escapeHTML(fParts[1])}</strong></div>
                </div>
                <div style="margin-top:15px; font-size:13px; font-weight:700; opacity:0.9; text-align:center;">
                    ${escapeHTML(fParts[3]) || "Check terminal for gate info"}
                </div>
            </div>`;
        }
...
async function loadItinerary() {
    try {
        ...
        // AUDITED: Better regex splitting handles normal CSV parsing safely.
...
function renderAccommodations() {
    ...
    cities.forEach(c => {
        ...
        const processCard = (f, acc) => {
            ...
            // AUDITED: Perfect Maps URL Format
