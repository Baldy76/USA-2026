(() => {
    ...
    function openTab(pageId, buttonId = null) { safeRun('Navigation', () => {
        ...
        // Dynamically update timezones immediately when tab changes
    ...
    // FEATURE: Dynamic Timezones based on active tab
    function updateTimeAndCountdown() { safeRun('Clocks', () => {
        ...
        // Dynamically shift Timezone based on the page user is viewing
        const activeTab = document.querySelector('.tab-content.active')?.id || 'home';
        let localTz = 'America/Los_Angeles'; // Default to PT
        let localTzLabel = '🇺🇸 Local (PT)';

        if (activeTab === 'utah') {
            localTz = 'America/Denver'; // Switch to MT
    ...
    async function preCacheImages() {
        if ('caches' in window) {
            try {
                const cache = await caches.open('holiday-planner-v2.1.4'); 
    ...
    function renderAccommodations() { safeRun('RenderAcc', () => {
        ...
        vaultAndStaysData.forEach(cols => {
            ...
            if (type === 'stay' && (filter === 'All' || fam.toLowerCase() === filter.toLowerCase() || fam.toLowerCase() === 'everyone')) {
                ...
                // Note: Replaced inline events with data-url for event delegation
                const ui = `
        ...
        itineraryData.forEach(cols => {
            if(cols.length >= 5) {
                ...
                if (filter === 'All' || who.toLowerCase() === filter.toLowerCase() || who.toLowerCase() === 'everyone') {
                    ...
                    // Added "itin-card" class to capture click events via event delegation
                    const cardHtml = `
                        <div class="admin-card itin-card" style="text-align: left; transition: all 0.3s ease; cursor: pointer; padding: 20px; margin-bottom: 16px;">
    ...
    // ==========================================
    // NEW: EVENT BINDING ENGINE (No Inline HTML Events)
    // ==========================================
    function bindEvents() {
        ...
        // Global Event Delegation for Dynamically Generated Content (Itinerary Cards & Map Links)
        document.body.addEventListener('click', (e) => {
            // Check if user clicked an itinerary card
            const itinCard = e.target.closest('.itin-card');
            if (itinCard) {
                // Ignore the click if they specifically clicked the map hyperlink
                if (e.target.tagName.toLowerCase() === 'a') return;
                toggleComplete(itinCard);
            }
    ...
    // ==========================================
    // 6. INITIALIZATION & PWA MAINTENANCE
    // ==========================================
    window.addEventListener('load', () => {
        ...
        // FEATURE: System-Synced Dark Mode
        safeRun('InitTheme', () => {
            const prefersDark = window.matchMedia('(prefers-color-scheme: dark)');
            const savedTheme = localStorage.getItem('HolidayPlanner_Theme');
            
            // If user never set a theme manually, use the system default
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            ...
            updateTimeAndCountdown(); // Ensure clocks correct themselves instantly upon app resume
        }
    });
})();
